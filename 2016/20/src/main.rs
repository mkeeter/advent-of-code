...
fn main() {
    ...
    for i in 0..blacklist.len() {
        for j in (i + 1)..blacklist.len() {
            ...
            if blacklist[j].0 >= blacklist[i].0 &&
               blacklist[j].1 <= blacklist[i].1
            {
                blacklist[j] = (0, -1);
            }
            // If j is partially above i, move it to be totally above
            else if blacklist[j].0 >= blacklist[i].0 &&
                    blacklist[j].0 <= blacklist[i].1
            {
                let v = blacklist[i].1 + 1;
                blacklist[j].0 = v;
            }
            // If j is partially below i, move it to be totally below
            else if blacklist[j].1 >= blacklist[i].0 &&
                    blacklist[j].1 <= blacklist[i].1
            {
